...
Rustの言語デザインやライブラリは安全性と高速性を確保しながらその範囲で**ゼロコスト抽象化**（_zero-cost abstraction_）を追求しています。

他の言語では『イテレータではなくfor文で書いた方が速い』、『async/awaitを使うと遅い』といったことは珍しくありません。
その場合も最適化よりは抽象化の方に倒すことが多いでしょう。
しかしボトルネックを高速化する必要があるという状況も出て来ます。
そうなると一般的にコードの見た目は悪くなりますし手間もかかります。

Rustでは抽象化とパフォーマンスを秤にかける機会は他の言語ほどは無いです。
『コスト』は基本的に可視化されているのでそこをちょっと変えるだけです。

また抽象化のために例えば次のような他のプログラミング言語に備わっている優れた機能を輸入しています。
...
- [代数的データ構造](https://doc.rust-lang.org/book/ch06-01-defining-an-enum.html)
    ...
    いわゆる直和型や代数的データ型と呼ばれるものです。

- [型クラス](https://doc.rust-lang.org/book/ch10-02-traits.html)

    Rustのトレイトは型クラスのように扱うことができます。（[参考](https://keens.github.io/slide/katakurasuwokoete/)）
    JavaやC#の『インターフェイス』と呼ばれるものよりはるかに柔軟な実装ができます。

    Rustにはいわゆる『オブジェクト指向言語』が持つ継承の仕組みはありませんが、その代わりに柔軟な列挙型とトレイトの仕組みを活用するのがRustの選択です。
    継承をしたくなったときは以下の方法があります。
    大抵はこのうち1.と2.で事足ります。
    （[参考](https://twitter.com/i/web/status/928550702720233472)）

    1. 実装クラスが数個かつパラメータも多くはない

        列挙型（直和型）を使います。
        そもそもこのような場合直和型が適切である場合が多いです。
        実際KotlinやScalaではJVMの上で抽象クラスとそのサブクラスを直和型のように扱うためにsealed classという機能を供えています。

    2. 実装の共有

        トレイトのデフォルト実装を使います。
        フィールドはgetterメソッドで得ます。
        むしろこれにより各フィールドやメソッドの役割が明確になり無意味に複雑なコードが生まれにくくなります。

        ただgetterを書くときに『1つのオブジェクトに対する可変参照は高々1つしか存在できない』という原則がほんの少しだけ邪魔になるかもしれません。

    3. 2.に加えインターフェイスごと雑に継承

        [`Deref`](https://doc.rust-lang.org/stable/std/?search=Deref)と[`DerefMut`](https://doc.rust-lang.org/stable/std/ops/trait.DerefMut.html)で中のメソッドを直で呼べます。
...
- [シャドーイング（の推奨）](https://doc.rust-lang.org/book/ch03-01-variables-and-mutability.html#shadowing)
    ...
    Rustではシャドーイングが禁止されたり警告されるどころか推奨されています。
    これにより変なprefixやsuffixが付いた変数も宣言しなくて良くなりますし`mut`を消せる場合も出てきます。
    静的型付け言語なのでどの変数がどの型なのかは書いた時点できちんと定まっています。
    エディタ上なら（大抵）一目でわかります。

    また変数の数を抑止するという効果もあります。
    move semanticsにより**減る**場合もあるので上記のblock expressionと合わせれば同じ関数の実装を長々と書いてもあまり汚いコードにはなりません。
    競技プログラミングでは尚嬉しいのではないでしょうか。

あとRust特有の機能もパフォーマンスと安全性のみならず抽象化にも寄与することがあります。
["Shared XOR mutable"](https://speakerdeck.com/qnighy/rust-imperative-language-2-dot-0?slide=29)（氏命名?）は不要な可変性を排除する助けになります。
ライフタイム自体やmove semanticsもコードリーディングの助けになることが（偶に）あります。
