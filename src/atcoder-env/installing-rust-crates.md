...
そしてジャッジの際には[以前説明した](./supported-environment.md#一般的なrustプログラム開発環境との違い)`cargo`を利用する方法と`rustc`を利用する方法のいずれかを利用してコンパイルします。
...
└-- WORKAREA               # ジャッジ用の一時ディレクトリ
    └-- main               # 全部入りプロジェクトをまるまるコピーしたもの
        |-- main.rs        # ユーザプログラム（提出されたプログラム）のソースコードに置き換える
        |-- Cargo.toml
        └-- target
            └-- release
                |-- deps   # (外部クレートのコンパイルキャッシュ)
                └-- main   # コンパイル、リンク済みの実行ファイルがここに生成される
```

### [rustc] AtCoderジャッジサーバでのファイルレイアウト（ジャッジの際に作成するもの）

`cargo`を使用せず、`rustc`に適切なオプション（ライブラリ検索パスなど）を与えて実行することで、事前にコンパイルしておいた`rlib`ファイルとリンクさせます。
...
それではインストール作業に入りましょう。実際のコンパイルに`cargo`を使う場合も`rustc`を使う場合もここは同様です。
